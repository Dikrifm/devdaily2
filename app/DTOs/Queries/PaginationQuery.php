...
final class PaginationQuery
{
    ...
    private function applyRequestParameters(array $requestData): self
    {
        ...
        if (isset($requestData['page'])) {
            $clone->page = self::parsePage($requestData['page']);
        }
        ...
        foreach ($perPageKeys as $key) {
            if (isset($requestData[$key])) {
                $clone->perPage = self::parsePerPage($requestData[$key], $clone->maxPerPage);
                break;
            }
        }
     ...
     * @return int
     */
    private static function parsePage($page): int
    {
        $page = (int) $page;
     ...
     * @return int
     */
    private static function parsePerPage($perPage, int $maxPerPage): int
    {
        $perPage = (int) $perPage;
    ...
    public function getCacheKey(?string $prefix = null): string
    {
        ...
        $key = sprintf('%s%d_%d', $prefix, $this->page, $this->perPage);
    ...
    {
        $prefix = $prefix ?? $this->cachePrefix;
        $key = sprintf('%s%d_%d_%d', $prefix, $this->page, $this->perPage, $totalItems);
    ...
    {
        $clone = clone $this;
        $clone->page = self::parsePage($page);
    ...
    {
        $clone = clone $this;
        $clone->perPage = self::parsePerPage($perPage, $this->maxPerPage);
    ...
    public function getMaxPerPage(): int { return $this->maxPerPage; }
    public function getDefaultPerPage(): int { return $this->defaultPerPage; }
    public function getCachePrefix(): string { return $this->cachePrefix; }
    ...
    {
        return sprintf('Page %d of %d (per page: %d)', 
            $this->page, 
            $this->totalPages ?? '?', 
            $this->perPage
        );
